...
def find_good_pairs(similarities, threshold, xross):
    ...
    * a dict of pairs as keys (values are just True) containing those
    ...
    for ((a, file_num), b), score in similarities.items():
        # print("a : ", a , "b : ", b , "file_num : ", file_num , "score: ", score)
        if score >= threshold:
            if b in ref_hits:
...
def neighborhood_correlation(id2accession, similar_pairs, n_queries, n_ref_seqs, threshold=0, xross=False, nc_thresh=0.05, output_file="python_output.txt"):
    '''
    ...
    `nc_thresh` - Neighborhood Correlation threshold for reporting results.
    `output_file` - Name of the file to write results to.
    '''
    ...
    print("similar pairs:", len(similar_pairs))
    ...
    with open(output_file, "w") as outfile:
        for a, b in good_pairs:
            ...
            if cor >= nc_thresh:
                outfile.write(f"{id2accession[a]} {id2accession[b]} {cor:.3f}\n")
            counter += 1
            if counter % print_interval == 0:
                elapsed_time = time.time() - start_time
                print(f"Iteration: {counter} - Time elapsed: {elapsed_time:.2f} seconds")
                start_time = time.time()  # Reset the start time for the next interval

    logging.info(f'NC computations completed. Results written to {output_file}')
...
def nc_main():
    ...
    if args.score_transform:
        transform = get_transform_function(args.score_transform)
    ...
    if args.three_col:
        ...
        id2accession, similarities, n_queries, n_ref_seqs, singletons = read_blast_tab(args.infile, transform)

    if singletons:
        logging.info(f'Noted {len(singletons)} sequences without a hit in the reference data.')

    if similarities:
        consider = transform(args.consider) if transform else args.consider

        neighborhood_correlation(id2accession, similarities, n_queries, n_ref_seqs,
                                 threshold=consider, xross=args.xross_files,
                                 nc_thresh=args.nc_thresh, output_file="python_output.txt")
